...
interface StretchStats {
    ...
    by_reason: Record<string, { total: number; wins: number; losses: number; pnl_cents: number }>;
}
...
function StatCard({
    ...
    return (
        ...
        >
            <div className="text-amber-600 text-sm mb-1 font-medium">
                {label}
            </div>
...
function PnlChart({ trades, balanceCents, portfolioCents }: { trades: Trade[]; balanceCents: number; portfolioCents: number }) {
    ...
    for (let i = 0; i < stepCount; i++) {
        ...
        if (i > 0) {
            points.push({ x, y: toY(steps[i - 1].value), value: steps[i - 1].value, label: "", date: null });
        }
        ...
        <div className="animate-fade-in gold-glow bg-zinc-900/80 border border-amber-900/30 rounded-xl p-5 mb-8 backdrop-blur-sm">
            ...
            <svg
                ...
                onTouchMove={(e) => {
                    ...
                    setHoverIdx(findClosest(((touch.clientX - rect.left) / rect.width) * w));
                }}
                onTouchEnd={() => setHoverIdx(null)}
            >
                <defs>
                    <linearGradient id="pnlGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={totalPnl >= 0 ? "#4ade80" : "#f87171"} stopOpacity="0.3" />
                        <stop offset="100%" stopColor={totalPnl >= 0 ? "#4ade80" : "#f87171"} stopOpacity="0.02" />
                    </linearGradient>
                </defs>
                ...
                {hp && (
                    <>
                        <line
                            ...
                            opacity="0.5"
                        />
                        <circle cx={hp.x} cy={hp.y} r="5" fill="#f0d060" stroke="#000" strokeWidth="1.5" />
                        <rect
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {[...games]
                    .sort((a, b) => {
                        ...
                        if (a.has_bet !== b.has_bet)
                            return a.has_bet ? -1 : 1;
        ...
        <div className="min-h-screen bg-black text-white p-6">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-8 animate-fade-in">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            <div
                                className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse shadow-lg shadow-amber-500/50"
                            />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 animate-fade-in">
                    <div className="lg:col-span-2 bg-zinc-900/60 border border-amber-900/20 rounded-xl p-6">
                        <h2 className="text-lg font-bold text-amber-400 mb-3">
                            The Strategy
                        </h2>
                        <div className="space-y-2 text-sm text-zinc-400">
                            <p>
                                Buy <span className="text-amber-300 font-semibold">YES contracts at 92c+</span> on
                                Kalshi sports markets when a team is winning by a comfortable margin in
                                the final minutes of the game.
                            </p>
                            <p>
                                ESPN live data confirms the game state — we only bet when the
                                outcome is nearly certain. Each $5 bet earns $0.05–$0.30 profit
                                in minutes.
                            </p>
                            <div className="grid grid-cols-2 gap-3 mt-4">
                                <div className="bg-black/40 rounded-lg p-3 border border-amber-900/10">
                                    <div className="text-amber-600 text-xs uppercase tracking-wider mb-1">Filters</div>
                                    <div className="text-zinc-300 text-xs leading-relaxed">
                                        YES price ≥ 92c<br />
                                        Final period only<br />
                                        ESPN-verified score lead<br />
                                        Volume ≥ 50 contracts
                                    </div>
                                </div>
                                <div className="bg-black/40 rounded-lg p-3 border border-amber-900/10">
                                    <div className="text-amber-600 text-xs uppercase tracking-wider mb-1">Risk Controls</div>
                                    <div className="text-zinc-300 text-xs leading-relaxed">
                                        Max $5 per bet<br />
                                        Max 20 concurrent positions<br />
                                        No duplicate event bets<br />
                    ...
                    <div className="bg-zinc-900/60 border border-amber-900/20 rounded-xl p-4 flex flex-col">
                        ...
                        <div className="flex-1 overflow-hidden [&_article]:!bg-transparent [&_article]:!border-amber-900/20 [&_.react-tweet-theme]:!bg-transparent" data-theme="dark">
                ...
                {/* P&L Chart */}
                <PnlChart trades={trades} balanceCents={stats.balance_cents} portfolioCents={stats.portfolio_value_cents} />

                {/* Stretch "What If" Stats */}
                {stretchStats && stretchStats.total > 0 && (
                    <div className="animate-fade-in bg-zinc-900/80 border border-amber-900/30 rounded-xl p-5 mb-8 backdrop-blur-sm">
                        <div className="flex justify-between items-center mb-3">
                            <h2 className="text-sm text-amber-600 font-medium">
                                What If? (Shadow Tracking)
                            </h2>
                            <span className="text-xs text-zinc-500">
                                Markets that nearly met our filters
                            </span>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-black/30 rounded-lg p-3 border border-zinc-800">
                                <div className="text-zinc-500 text-xs">Tracked</div>
                                <div className="text-amber-200 text-lg font-bold">{stretchStats.total}</div>
                                <div className="text-zinc-600 text-xs">{stretchStats.open} pending</div>
                            </div>
                            <div className="bg-black/30 rounded-lg p-3 border border-zinc-800">
                                <div className="text-zinc-500 text-xs">Shadow W/L</div>
                                <div className="text-lg font-bold">
                                    <span className="text-green-400">{stretchStats.wins}W</span>
                                    {" / "}
                                    <span className="text-red-400">{stretchStats.losses}L</span>
                                </div>
                                <div className="text-zinc-600 text-xs">
                                    {stretchStats.win_rate > 0 ? `${stretchStats.win_rate}% win rate` : "no settlements yet"}
                                </div>
                            </div>
                            <div className="bg-black/30 rounded-lg p-3 border border-zinc-800">
                                <div className="text-zinc-500 text-xs">Hypothetical P&L</div>
                                <div className={`text-lg font-bold font-mono ${stretchStats.hypothetical_pnl_cents >= 0 ? "text-green-400" : "text-red-400"}`}>
                                    {stretchStats.hypothetical_pnl_cents >= 0 ? "+" : ""}{cents(stretchStats.hypothetical_pnl_cents)}
                                </div>
                                <div className="text-zinc-600 text-xs">if we'd traded these</div>
                            </div>
                            <div className="bg-black/30 rounded-lg p-3 border border-zinc-800">
                                <div className="text-zinc-500 text-xs">By Reason</div>
                                <div className="space-y-1 mt-1">
                                    {Object.entries(stretchStats.by_reason).map(([reason, data]) => (
